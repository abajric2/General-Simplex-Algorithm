...
function general_simplex(goal, c, A, b, csigns = nothing, vsigns = nothing)  
    if csigns === nothing
        ...
        if goal == "max"
            # Ako je cilj maksimizacija, invertiraj sve znakove u matrici
            csigns .= -csigns
        end
    end 
    if vsigns === nothing
        vsigns = ones(Int, 1, size(c, 2))
    end 
    ...
    indeksi_spregnutih_sa_izvornim_y = Vector{Int}()
    ...
    for i=1:size(A)[1] #priprema koeficijenata dopunskih i vještačkih za simpleks; pamćenje koje su dopunske a koje vještačke
        if (csigns[i] == -1)
            ...
            push!(indeksi_spregnutih_sa_izvornim_y, broj_promjenljivih)
        elseif (csigns[i] == 0)
            broj_promjenljivih += 1
            A[i, broj_promjenljivih] = 1
        ...
        elseif (csigns[i] == 1)
            ...
            push!(baza, broj_promjenljivih)
            push!(indeksi_spregnutih_sa_izvornim_y, broj_promjenljivih - 1)
        end
    end
    ...
    while (maximum(T[size(T)[1], setdiff(1:size(T)[2]-1, indeksi_vjestackih)]) > 0.00000001 || maximum(T[size(T)[1] - 1, setdiff(1:size(T)[2]-1, indeksi_vjestackih)]) > 0.00000001) # Provjera da li u zadnjem redu (bez zadnje kolone) ima elemenata > 0
        ...
        redZ = false
        if all(x -> !(x in baza), indeksi_vjestackih) # provjera da li su sve vjestacke napustile bazu; ako jesu ne gleda se više red M
            redZ = true
    ...
    Yi = zeros(1, size(indeksi_spregnutih_sa_izvornim_y)[1]) #rjesenja za izvorne Y
    Yd = zeros(1, pocetni_broj_promjenljivih) #rjesenja za dopunske Y
    for i=1:size(Yi)[2]
        idx = indeksi_spregnutih_sa_izvornim_y[i]
        value = T[size(T)[1]-1, idx]
    ...
    for i=1:size(Yd)[2]
        Yd[i] = T[size(T)[1]-1, i]
        if(Yd[i] != 0) 
            Yd[i] *= -1 #ispisuje -0 ako se ovo ne uradi
        end
    end
    for i = 1:size(baza)[1]
    ...
    Xi = x[1:pocetni_broj_promjenljivih]
    Xd = x[pocetni_broj_promjenljivih+1:end]
    return cilj, Xi, Xd, Yi, Yd, status # x-evi i vrijednost cilja
